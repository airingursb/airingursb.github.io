...
## Post Detail — Blog Pet (SHU-818 circular) (`/posts/{any-slug}/`)

- [ ] DOM: `#blog-pet` 存在且桌面端可见（宽度 > 800px），位于右下角
- [ ] DOM: `#blogPetCanvas` 存在
- [ ] Evaluate: 模拟 `pointermove` 绕圈移动后 canvas 仍可见，无 JS 报错，转头连续无扇区跳变
- [ ] Screenshot: 右下角水墨熊猫截图，确认透明背景与 360° 转头动画
...
- [ ] Blog / Moment / Notes / Reading / Photos / Friends 及其详情页、中英文路由均通过 `SiteHeader.astro` 渲染统一导航，包含 Reading 入口
- [ ] 375 / 768 / 1280px 下字体、页边距、顶线和导航尺寸一致；移动菜单可展开所有入口，Escape 收起后焦点回到按钮
- [ ] 英文导航跳往对应 `/en/` 路由，Photos 保持 `/photos/`；嵌套页高亮所属栏目，无英文翻译的文章不生成错误语言入口
- [ ] 周刊归档/详情使用相同 header；博客日期、文章数、在线人数、运动详情返回入口仍保留

### Reading Weekly archive (`/reading/weekly/`)

- [ ] Reading 详情底部“更新/更早一条”卡片，中英文、375/768/1280px 下均使用完整 3:2 横向缩略图；图片高度随宽度缩放，不继承 240px 高度、不裁成竖条；长标题不造成横向溢出

- [ ] GET `/reading/weekly/`、`/reading/weekly/2026-w36/` 及对应 `/en/` 路由均返回 200，canonical 与中英 `hreflang` 正确
- [ ] Reading 列表页的 Weekly letter 区同时提供周刊归档入口与订阅管理入口，中英文目标正确
- [ ] 归档页区分周刊名称与本期主题，展示封面、推荐语和三条专题入口；不展示内部编辑流程
- [ ] 归档专题入口和单期目录能跳到对应章节；手机普通读卡使用小图，正文推荐语恢复全宽
- [ ] 主题切换按钮沿用全站位置，不覆盖 header 导航或语言切换；手机左右页边距一致
- [ ] 归档与单期页均可打开 Reading 订阅弹窗，Escape 关闭后焦点返回入口；不在 UI 测试中实际提交订阅
- [ ] 草稿期显示“编辑预览”，不得伪装成已刊发内容；没有明确人工确认时不进入正式发布状态
- [ ] 单期页完整显示期号、日期、编者手记、头条、3 个主题章节和 8 篇精选；逐条推荐语与每日摘要有明确视觉区分
- [ ] 周刊封面全部请求构建期 WebP 派生图，不直接把 R2 原图作为列表或正文图片加载
- [ ] 375 / 768 / 1280px 下归档页和单期页均无横向溢出；中文标题无单字孤行，按钮与链接键盘焦点清晰
- [ ] 周刊期次、条目与来源跳转携带 Reading Umami 事件，且事件属性不含邮箱或其他个人信息
...
## 日常箱庭 Demo (`/diorama/rainy-marina-bay/`)

- [ ] No.16《雨里的滨海湾》的第三格被真实 3D 场景还原，含熊猫、Moflow、两辆自行车、公交站、海湾与远处金沙
- [ ] 可拖动旋转、缩放，按钮及键盘方向键 / +/- / 0 可操作，恢复视角有效
- [ ] 3 个场景编号与下方记忆选项同步选中状态和文案；所有操作可通过键盘访问
- [ ] 暂停冻结雨滴和涟漪；系统减少动态时默认冻结；雨声默认关闭，按钮可开启与关闭
- [ ] 展开原故事可查看真实四格、原文及原作链接；无 WebGL 时有说明且原故事仍可阅读
- [ ] 375 / 768 / 1280px 无横向溢出、中文裁切或控件遮挡，Canvas 随容器自适应
- [ ] 主角细化：熊猫眼斑贴合头部、弯曲手臂和脚掌在旋转后仍完整；Moflow 是连续水滴轮廓，爱心和手指不脱离身体
- [ ] 主角细化：放大后可辨识眼睛高光、熊猫口鼻与舌头、Moflow 微笑和抱心姿态；恢复视角与手机布局仍可用

### 独立毛绒角色模型 /diorama/characters/
- [ ] 一起坐坐 / 熊猫 / Moflow 切换正确，独立角色居中，模型不是静态图片
- [ ] 正面、侧面、背面均可查看；鼠标拖动、键盘旋转、缩放与复位有效
- [ ] 眼斑和肚皮毛色分区正确；毛发没有拉长到原点，侧背面无脱落五官或断裂手臂
- [ ] 375 / 768 / 1280px 布局和操作可用，下载链接指向实际独立 GLB 文件
- [ ] 前往箱庭再返回时模型能继续显示；无法启动 WebGL 时有文字说明且下载仍可用
- [ ] 第二版：低矮坐姿、立体双瓣口鼻、张嘴吐舌、Moflow 厚爱心与圆润抱心手掌，在正面和侧面均保持完整
- [ ] 第二版：加载和下载均指向 v2 GLB；毛发双面着色不产生明显反面灰斑，角色侧背面仍正确

- [ ] 第三版熊猫：小眼睛贴合眼斑，抬高的口鼻、舌根遮挡在正面/侧面都自然；Moflow 保持 v2，熊猫加载和下载使用 v3

- [ ] 箱庭暴雨细化：金沙三座弧形塔楼、悬挑船形天台、泳池与立面楼层可辨识；斜雨、屋檐流水和落地水花同时可见，屋檐内没有雨穿过主角
- [ ] 箱庭暴雨细化：暂停冻结雨线/水花/涟漪，继续后恢复；手机视口可操作，减少动态默认暂停

- [ ] 箱庭载入熊猫 v3 / Moflow v2 毛绒 GLB，角色坐在长椅上，正面与转动后不穿出屋顶/后墙；加载失败有说明，离开后到达的模型资源释放
- [ ] 细雨版雨线短而稀疏、屋檐水帘减弱，暂停/继续和减少动态仍控制全部雨效

## 日常箱庭收藏入口

- [ ] 首页四格模块之后显示日常箱庭横向收藏架，最多八件；“查看全部”打开 `/diorama/`。
- [ ] 中文博客 `/blog/` 不展示箱庭收藏架；个人主页入口明暗模式文字可读。
- [ ] `/diorama/` 显示真实收藏数、故事日期、地点、封面和原四格；只有一件时无虚构卡片或无效筛选。
- [ ] 列表和首页只加载静态封面，不请求角色 GLB 或启动 WebGL。
- [ ] 封面 / 标题 / “走进这一天”打开 `/diorama/rainy-marina-bay/`；详情返回全部箱庭，原四格仍可访问。
- [ ] `/comics/16/` 显示关联箱庭入口；没有关联箱庭的四格不显示此入口。
- [ ] 桌面、平板、375px 手机下入口、列表、详情、四格入口无横向溢出，标题不出现孤字断行，键盘焦点可见。
- [ ] 为新场景添加独立页面、真实封面和 `src/data/dioramas.ts` 条目后，首页最近八件及列表自动收录。

## 釜山生日夜海与入口调整

- [ ] 个人主页显示两件箱庭；中文博客页不再展示日常箱庭模块；收藏列表的返回链接指向个人主页。
- [ ] `/diorama/busan-birthday-sea/` 为真实3D：开放海面、红灯塔、防波堤、小渔船、三间暖灯夜市店铺、海鲜车；没有雨和金沙酒店。
- [ ] 熊猫v3与Moflowv2坐在码头边，主角面部不被记忆标记遮挡；近景可见鱼、蟹、贝壳、遮棚条纹、灯串和招牌。
- [ ] 海浪和小船轻轻运动；暂停后波浪、小船、灯塔光点和反光冻结；减少动态时默认冻结且继续按钮禁用。
- [ ] 海浪音效默认关闭，开启/关闭有正确状态和文案；音效明确为合成海浪。
- [ ] 三个记忆同步更新标题/正文；原故事与四格15正确，四格15回链到釜山，四格16仍回链滨海湾。
- [ ] 列表两列真实收藏，按故事日期排列；个人主页紧凑横向卡片；列表手机单列无溢出。
- [ ] 共享详情迁移后滨海湾仍正常加载，原先雨声/细雨/三段记忆不混入釜山内容。
- [ ] 可见浪峰：白色浪花随真实起伏海面向岸边推进，贴合水面，不穿进码头；小船随所在位置波高上下浮动，暂停后全部冻结。

### 首页箱庭紧凑横向入口
- [ ] 首页箱庭沿用四格标题栏和卡片宽度，单行横向排列；不出现长描述、标签或大幅 CTA。
- [ ] 375px 手机下可水平滑到第二件收藏，标题和日期清楚，无整页横向溢出；Tab 可聚焦并露出第二张卡片。
- [ ] 桌面与平板维持紧凑单行；封面/标题链接分别进入对应箱庭，查看全部进入收藏列表。
- [ ] 箱庭列表仍保留完整介绍；博客页无箱庭入口。

### 首页横向滚动条
- [ ] 阅读流、四格、箱庭、照片和运动横向列表使用透明轨道与细圆角灰色滑块，深色模式没有白底滚动条。
- [ ] 明暗模式切换后所有横向滚动条同步适配；悬停/键盘焦点时滑块仍清晰，内容布局不变。
- [ ] 手机/桌面可继续横向滑动或用方向键浏览后续卡片，整页不横向溢出；高对比模式保留系统滚动条颜色。

### 博客侧栏箱庭预览
- [ ] 中英文博客作者栏订阅框下显示釜山箱庭002，原盆栽 iframe 不再加载；模型有真实熊猫/Moflow、灯塔和夜市。
- [ ] 模型与标题整体可点击/按Enter进入釜山详情，Canvas不进入Tab顺序，页面滚轮不被模型接管。
- [ ] 默认静止，悬停/键盘聚焦时海浪播放，离开后冻结；减少动态保持静止，离开可见区暂停。
- [ ] 1200px以下隐藏并释放预览，手机初次打开不创建Canvas；桌面恢复后模型可重新加载。
- [ ] 加载时有封面提示，模型加载失败保留可用的封面/详情链接；原箱庭详情旋转/缩放/播放不变。

### 新箱庭：南部山脊与小区桑拿房
- [ ] `/diorama/southern-ridges-sky/` 加载真实模型：熊猫躺在木桥上、Moflow 在身上，林间松鼠和三只飞鸟可见；不会误载滨海湾雨景。
- [ ] `/diorama/hidden-sauna/` 加载木屋剖面、长凳、石炉热气、木桶、小灯和门口的 Moflow；不会误播放雨声或海浪声。
- [ ] 两个新详情页的三个记忆点、原四格展开/链接、缩放、重置和键盘控制正常；暂停冻结飞鸟/热气，减少动态效果设置优先。
- [ ] 375 / 768 / 1440 宽度下模型完整、中文标题无单字孤行、控制按钮可点、页面没有横向溢出。
- [ ] 个人主页横滑列表与箱庭收藏显示四件收藏及真实场景封面；第 8 / 17 话原漫画出现对应箱庭入口。
- [ ] 已有滨海湾、釜山详情与博客侧栏釜山预览保持原有构图和交互。

### 箱庭姿态与森林细化
- [ ] 仰望使用专门的仰卧熊猫与仰面 Moflow，手脚舒展、脸部无遮挡，身体贴合木桥与肚子；桑拿熊猫后靠、Moflow 抬手，旧场景保持原姿态。
- [ ] 森林木纹、细叶树冠、蕨叶与苔地在缩放后仍有细节，默认桌面/手机视角包含完整树冠和三只飞鸟，记忆点不遮脸。
- [ ] 更新后的两张收藏封面与详情一致；暂停和系统减少动态效果仍能冻结飞鸟及热气。

### Pokopia 风格箱庭试版
- [ ] 《仰望》与《桑拿房》的角色表面光滑、姿态各不相同，脸与爱心清晰；旧滨海湾/釜山及角色工作室保留毛绒材质。
- [ ] 森林呈现饱满树冠、厚草簇与分层草土地形，桑拿房呈现明亮木材、青绿地砖和圆润植物；375/768/1280 宽度默认视角中主体完整。
- [ ] 旋转、缩放、重置、记忆点、暂停和减少动态效果正常；角色加载失败仍可阅读原漫画；列表封面对应实际新场景。

### 《仰望》飞鸟动画
- [ ] 默认打开页面，三只鸟在树冠上方沿不同相位绕行，飞行方向与身体朝向一致，有明显扇翅与滑翔；手机上可辨认且完整一圈不飞出默认画面。
- [ ] 点击定格后鸟的位置和翅膀完全停止，继续后衔接原位置；减少动态效果启用时保持静止；树冠没有脱离鸟体的固定鸟影。
...
### V9 窗边访客与信箱（本地预览）
- [ ] 新鲜晴/阴天气、白天、窗帘开启且首屏可见时，停留20–40秒可遇见小鸟；点击仅歪头/跳动，无字幕、导航或声音。
- [ ] 窗帘关闭、进入夜晚、天气过期/下雨、打开预览弹层时，小鸟取消；顶部与页尾不同时出现，冷却时间共享。
- [ ] 首次等待期间滚离顶部再回来，已等待时间保留；切换标签页时等待/访问暂停，回来继续，不因后台错过小鸟。已出现的小鸟滚离视窗则离开，允许它之后到页尾。
- [ ] 本地场景演示注入 data-bird-preview 时约3秒出现；正常页面仍为20–40秒，不从网址参数开启快速模式。
- [ ] 减少动态模式不自动出现小鸟；窗帘、书摘、订阅仍可通过键盘操作。
- [ ] 关帘只使桌边收暗，台灯开启仍保留暖光；网站主题不变。
- [ ] 晴天云朵与雨天溅水仅在窗玻璃内；有效雨停后余滴12秒，暂停恢复正确，过期天气清除余滴。
- [ ] 页尾信箱与订阅展开状态同步；开箱不发送请求；提交等待、失败保留输入、重试成功投信及小熊回应均正确。
- [ ] 375/768/1280明暗视图无新增溢出，鸟脚接触窗台/树枝，按钮有44px点击区域及可见键盘焦点。
- [ ] 后续美术确认后的H3专项：小鸟啄玻璃后，小熊等待当前动作安全结束再抬头，用户主动交互优先，结束后回原活动。（当前预览未接入此动作）
- [ ] 同浏览器连续两个新加坡日期访问，第二天等待10–20秒；同日刷新保留，间隔一天恢复普通20–40秒；存储不可用不影响小鸟正常出现。
