...
# ///
# How to run: uv run scripts/bear-stories/garden/footer-register.py
...
def main() -> None:
    paths = sorted((ROOT / "h3/raw").glob("frame-*.png"))
    reference = read_image(ROOT / "reference/h3-start.jpg")
    ...
    cv2.imwrite(str(ROOT / "h3/temporal-plate.png"), plate)
    (ROOT / "h3/registration.json").write_text(json.dumps([asdict(item) for item in registrations], indent=2), encoding="utf-8")
