...
## Editorial footer studio (September 17)

Approved for implementation and local acceptance: a compact archive bookcart in Blog CN/EN, the existing watercolor panda docking at the article coda, prev/next paper previews, a newspaper subscription counter, and low-probability authored bookcart mishaps. Preserve the personal homepage garden and existing content layout. The brown pixel bear belongs to the bookcart/counter; the existing watercolor panda retains its identity in article pages. Generated art is only actors/props, never UI text or screenshots. These Astro surfaces use native DOM and CSS/WAAPI, without new React hydration or tooling dependencies.

### Reusable primitives and states

- EditorialActor: lazy transparent atlas and static poster in a reserved aspect-ratio box; states poster/loading/playing/quiet/paused/unavailable. Use new H3 footage for whole character gestures, not CSS body deformation. Bookcart visual footprint 208×128 desktop, 168×104 mobile; counter 112×96 desktop, 88×76 mobile; dock 96×96. Logical asset canvas may contain safety padding. Intrinsic image dimensions reserve space, and no animation moves adjacent content.
- ArchiveBookcart: neutral rule-separated footer, small serif invitation, mono year selectors and real article links from the content collection. A year button discloses that year's real list; links remain usable without JS. The bear's arrival is once per page; one optional mishap after 18–35 seconds of cumulative visible idle at 25% visit probability. Clicking the actor requests one complete performance, never queues repeats. Use paper/bookmark/bird physical acting with an authored return to rest. No visible playback menu, speech bubble or sound.
- ArticleReadingDock: one article pet identity, no simultaneous floating and docked pet. Near the coda the existing floating pet settles into the reserved inline dock; scrolling back restores pointer-following presence. Mobile uses only the inline version, respecting a stored dismissal. Reaching the coda triggers a fictional close-book action once; never labels the article actually read. Reduced motion shows a static dock without travel. Disable hidden/background playback.
- NextPagePreview: two real chronological neighbors, localized labels, real cover/description/date when available and a typographic fallback when no cover. Semantic disclosure control and separate native reading link work on keyboard/touch; desktop hover/focus may reveal. No fabricated excerpt. Keep cover-transition data attributes on actual links. Paper surface uses site background, rule and text tokens, with two offset border layers of 3px and 6px to suggest stacked pages. Border layers are noninteractive and decorative. When only one chronological neighbor exists, center its paper within the reading column at a maximum 420px width; two neighbors keep the paired grid.
- NewspaperSubscription: reuse the real subscribe endpoint, one form per placement, explicit label/autocomplete/email validation, submitting/confirmed/already subscribed/error states. Pending submission is single-flight. H3 clip-tab gesture only after a successful HTTP response and successful application result; existing subscription is not a new event. Never send a real subscription during QA; intercept the request at the browser boundary. Keep input and readable error on failure, live status, restore usable controls. Exact CN/EN strings are DOM. RSS is a native localized link. Preserve the existing service payload shape.

### Tokens and motion

Use existing neutral colors, accent, fonts and 4px spacing above. Footer shell remains 1200px, 48px/20px gutters; article sections retain the established reading column. New tokens: --editorial-reveal:240ms; --editorial-dock:420ms; --editorial-ease:cubic-bezier(.2,.8,.2,1); --editorial-actor-width:208px; --editorial-counter-width:112px; --editorial-dock-size:96px. UI type uses existing 12/14/16/18px roles; fold details inherit border colors. No large colored panels, giant footer artwork or postcard wall.

Interaction reference: beui.dev preview-rail source consulted 2026-09-17. Adapt its separate hover/focus/touch-pinned preview states and outside/Escape dismissal to native disclosure and anchors; do not vendor React/Motion. Reduced motion eliminates translations/folds/travel while retaining all links, disclosure, form results and poster states. H3 performances are explicitly user-requested ambient narrative, allowed inside these bounded scenes. Visible elapsed time pauses offscreen/hidden instead of resetting. Media errors leave a complete still; assets decode on demand with a bounded cache. Each action may name its own forward-playing quiet rest clip, preserving the resulting prop positions. Keep the last canvas frame visible while the next asset decodes; never flash back to the poster between performances. No requests to generative models in the visitor's browser.

### Evidence and constraints

Personas: first-time CN reader, returning EN reader, narrow-screen touch reader, keyboard/reduced-motion reader. Capture Blog CN/EN, article CN/EN, and component state harness at 375/768/1280 in light/dark. Test real navigation, retained article transition, disclosure, dock enter/exit, click replay, no duplicate pet, success/error/already-subscribed form responses, hidden/resume and resource failure. New H3 frames must retain complete ears, limbs, books and cart wheels; inspect temporal contact sheets over both backgrounds for colored/white matte, holes and drift. No deployment approval is implied by this implementation request.

### Editorial footer revision 2 (September 19)

The subscription is a compact receiving letter: a quiet paper surface, serif invitation, small H3 bear at its upper right, one address line and a postage-like submit control. Remove the outer horizontal section rules. The counter occupies 128×112px desktop and 104×96px mobile; it tidies mail, blinks and greets while visible, pauses offscreen/hidden, and keeps a complete poster under reduced motion or failed media. Only confirmed new subscriptions trigger the authored clipping gesture and the small receipt animation; duplicate, pending and error states remain truthful. Use the existing site palette, mono labels and 44px controls. Success text is announced and the form remains keyboard accessible. No added sound, playback menu or decorative text baked into media.

Neighbor navigation uses the reading column, quiet top rules and real linked titles with optional small covers, rather than offset paper outlines. Hover/focus or an explicit disclosure reveals description/date; touch preview stays separate from navigation. The coda is compact and retains one existing panda identity. Motion is limited to actual disclosure and submission state changes.

The monthly-36 article hero reuses the existing palace-clouds clip, automatically playing one silent visible cycle without hover. It pauses offscreen/hidden and resumes the remaining cycle. Reduced motion, Save-Data and media failure retain the poster. Preserve one cover transition target; blog-card hover/touch behavior is unchanged. All revised surfaces must fit CN/EN at 375/768/1280px in light/dark themes.
