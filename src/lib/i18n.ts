export type Lang = 'zh' | 'en';

// ============================================================
// Site-wide namespaced strings (new — used by all non-blog modules)
// ============================================================

export const siteStrings = {
  zh: {
    common: {
      home: 'Home',
      blog: 'Blog',
      archive: 'Archive',
      moments: 'Moment',
      photos: 'Photos',
      comics: 'Comics',
      notes: 'Notes',
      reads: 'Reads',
      friends: 'Friends',
      search: 'Search',
      admin: 'Admin',
      rss: 'RSS',
      brand: "Airing's Blog",
      copyright: '© 2026 Airing',
    },
    notes: {
      siteTitle: '思考的痕迹',
      siteDesc: '记录关于前端、数学、设计、哲学和一切让我好奇的事物',
      countSuffix: '篇笔记',
      allTag: '全部',
      searchPlaceholder: '搜索笔记标题或摘要…',
      emptyState: '没有找到匹配的笔记',
      backToList: '← 全部笔记',
      toc: '目录',
      tagsLabel: '标签',
      relatedNotes: '相关笔记',
      readingTime: (n: number) => `约 ${n} 分钟`,
      weekTooltip: (m: number, d: number, count?: number) =>
        `${m}/${d} 周` + (count ? ` (${count} 篇)` : ''),
      feedTitle: "Airing's Notes",
      feedDesc: '思考的痕迹：记录关于前端、数学、设计、哲学和一切让我好奇的事物。',
      feedLang: 'zh-CN',
    },
    reads: {
      siteTitle: '阅读',
      siteDesc: '公开的阅读摘录与外发摘要。',
      countSuffix: '篇',
      allSource: '全部',
      emptyState: '还没有公开的阅读。',
      emptyMonth: '暂无月份',
      backToList: '← 全部阅读',
      readOriginal: '阅读原文',
      feedTitle: "Airing's Reads",
      feedDesc: '公开的阅读摘录与外发摘要。',
      feedLang: 'zh-CN',
    },
    archive: {
      title: 'Archive',
      desc: (n: number) => `全部 ${n} 篇文章，按年份排序。`,
      dateLocale: 'zh-CN',
    },
    search: {
      title: 'Search',
      desc: '全文搜索所有文章。',
    },
    tags: {
      title: (tag: string) => `#${tag}`,
      desc: (n: number) => `该标签下共 ${n} 篇文章。`,
      backToBlog: '← 返回博客',
    },
    friends: {
      title: 'Friends',
      desc: '朋友们的博客，以及留言板。',
      lastChecked: (d: string) => `上次检测：${d}`,
      guestbook: '留言板',
    },
    moments: {
      title: 'Moments',
      desc: '碎片想法、日常记录。',
      dateLocale: 'zh-CN',
    },
    comics: {
      title: '四格',
      desc: 'by Airing & Moflow — 一周一遭，记录趋势与情绪。',
      archiveLabel: '— 往期 —',
      issuePrefix: 'No.',
      source: '源文本',
      prev: '上一期',
      next: '下一期',
      shareCopy: '复制链接',
      emptyState: '还没有任何一期。',
    },
    workouts: {
      title: 'Workouts',
      desc: '运动记录 · 徒步、跑步与漫游的轨迹',
      totalsLabel: (count: number, km: number, hours: number) =>
        `${count} 次 · ${km.toFixed(1)} km · 累计 ${hours.toFixed(1)}h`,
      emptyState: '还没有运动记录。',
      backToList: '← 全部运动',
      metricLabels: {
        pace: 'Pace',
        elevation: 'Elevation',
        heart: 'Heart',
        calories: 'Calories',
        steps: 'Steps',
        gps: 'GPS',
      },
      units: {
        distanceKm: 'km',
        paceSlash: 'km/h',
        elevUp: 'm',
        elevDown: 'm',
        bpm: 'bpm',
        bpmAvg: 'avg',
        bpmMax: 'max',
        kcal: 'kcal',
        kcalPerHr: '/hr',
        stepsPerMin: '/min',
        metresAvg: 'm avg',
        metresMin: 'm min',
      },
      detailLabels: {
        avgPace: 'Avg Pace',
        maxPace: 'Max Pace',
        vertical: 'Vertical (↑)',
        metres: 'Metres',
        gainLoss: 'Gain/Loss',
        ascent: 'Ascent',
        descent: 'Descent',
        bpm: 'BPM',
        avgBpm: 'Avg BPM',
        maxBpm: 'Max BPM',
        zones: 'Zones',
        total: 'Total',
        burnPerHr: 'Burn/hr',
        resting: 'Resting',
        cadence: 'Cadence',
        stepLength: 'Length',
        avgAcc: 'Avg Acc',
        minAcc: 'Min Acc',
        gpsDist: 'GPS Dist',
        map: 'Map',
      },
      dateLocale: 'zh-CN',
    },
    admin: {
      title: 'Admin',
      desc: '管理入口。',
    },
  },
  en: {
    common: {
      home: 'Home',
      blog: 'Blog',
      archive: 'Archive',
      moments: 'Moment',
      photos: 'Photos',
      comics: 'Comics',
      notes: 'Notes',
      reads: 'Reads',
      friends: 'Friends',
      search: 'Search',
      admin: 'Admin',
      rss: 'RSS',
      brand: "Airing's Blog",
      copyright: '© 2026 Airing',
    },
    notes: {
      siteTitle: 'Traces of Thought',
      siteDesc: 'Notes on frontend, math, design, philosophy, and everything that sparks my curiosity.',
      countSuffix: 'notes',
      allTag: 'All',
      searchPlaceholder: 'Search notes by title or summary…',
      emptyState: 'No notes match your filter',
      backToList: '← All Notes',
      toc: 'Contents',
      tagsLabel: 'Tags',
      relatedNotes: 'Related',
      readingTime: (n: number) => `${n} min read`,
      weekTooltip: (m: number, d: number, count?: number) =>
        `Week of ${m}/${d}` + (count ? ` (${count})` : ''),
      feedTitle: "Airing's Notes",
      feedDesc: 'Notes on frontend, math, design, philosophy, and everything that sparks my curiosity.',
      feedLang: 'en',
    },
    reads: {
      siteTitle: 'Reads',
      siteDesc: 'Public reading notes and outbound summaries.',
      countSuffix: 'reads',
      allSource: 'All',
      emptyState: 'No public reads yet.',
      emptyMonth: 'No month',
      backToList: '← All Reads',
      readOriginal: 'Read original',
      feedTitle: "Airing's Reads",
      feedDesc: 'Public reading notes and outbound summaries.',
      feedLang: 'en',
    },
    archive: {
      title: 'Archive',
      desc: (n: number) => `All ${n} posts, sorted by year.`,
      dateLocale: 'en-US',
    },
    search: {
      title: 'Search',
      desc: 'Full-text search across all posts.',
    },
    tags: {
      title: (tag: string) => `#${tag}`,
      desc: (n: number) => `${n} posts tagged.`,
      backToBlog: '← Back to Blog',
    },
    friends: {
      title: 'Friends',
      desc: "Friends' blogs and guestbook.",
      lastChecked: (d: string) => `Last checked: ${d}`,
      guestbook: 'Guestbook',
    },
    moments: {
      title: 'Moments',
      desc: 'Fragments of thought and daily notes.',
      dateLocale: 'en-US',
    },
    photos: {
      title: 'Photos',
      desc: 'A small collection of photographs.',
      noPhotos: 'No photos yet.',
      filterAll: 'All',
      exifFallback: '—',
    },
    comics: {
      title: 'Comics',
      desc: 'by Airing & Moflow — a weekly strip on trends and moods.',
      archiveLabel: '— Past Issues —',
      issuePrefix: 'No.',
      source: 'Source',
      prev: 'Previous',
      next: 'Next',
      shareCopy: 'Copy link',
      emptyState: 'No issues yet.',
    },
    workouts: {
      title: 'Workouts',
      desc: 'Workout log · hikes, runs, and wandering routes',
      totalsLabel: (count: number, km: number, hours: number) =>
        `${count} workouts · ${km.toFixed(1)} km · ${hours.toFixed(1)}h total`,
      emptyState: 'No workouts logged yet.',
      backToList: '← All workouts',
      metricLabels: { pace: 'Pace', elevation: 'Elevation', heart: 'Heart', calories: 'Calories', steps: 'Steps', gps: 'GPS' },
      units: { distanceKm: 'km', paceSlash: 'km/h', elevUp: 'm', elevDown: 'm', bpm: 'bpm', bpmAvg: 'avg', bpmMax: 'max', kcal: 'kcal', kcalPerHr: '/hr', stepsPerMin: '/min', metresAvg: 'm avg', metresMin: 'm min' },
      detailLabels: { avgPace: 'Avg Pace', maxPace: 'Max Pace', vertical: 'Vertical (↑)', metres: 'Metres', gainLoss: 'Gain/Loss', ascent: 'Ascent', descent: 'Descent', bpm: 'BPM', avgBpm: 'Avg BPM', maxBpm: 'Max BPM', zones: 'Zones', total: 'Total', burnPerHr: 'Burn/hr', resting: 'Resting', cadence: 'Cadence', stepLength: 'Length', avgAcc: 'Avg Acc', minAcc: 'Min Acc', gpsDist: 'GPS Dist', map: 'Map' },
      dateLocale: 'en-US',
    },
    admin: {
      title: 'Admin',
      desc: 'Admin panel.',
    },
  },
} as const;

export function tSite(lang: Lang) {
  return siteStrings[lang];
}

// ============================================================
// Blog strings (existing — backward compat for blog module only)
// ============================================================

export const blogStrings = {
  zh: {
    motto: '应无所住而生其心',
    posts_count: (n: number) => `${n} 篇文章`,
    popular: '热门文章',
    latest: '最新文章',
    read_more: '阅读更多',
    min_read: (n: number) => `${n} 分钟阅读`,
    series_names: { weekly: '月刊', annually: '年报' } as Record<string, string>,
    cn_only_badge: '',
    related_posts: '相关文章',
    prev_post: '上一篇',
    next_post: '下一篇',
  },
  en: {
    motto: 'Act without attachment',
    posts_count: (n: number) => `${n} posts`,
    popular: 'Popular',
    latest: 'Latest',
    read_more: 'Read more',
    min_read: (n: number) => `${n} min read`,
    series_names: { weekly: 'Weekly', annually: 'Annual' } as Record<string, string>,
    cn_only_badge: 'CN',
    related_posts: 'Related Posts',
    prev_post: 'Previous',
    next_post: 'Next',
  },
} as const;

export function t(lang: Lang) {
  return blogStrings[lang];
}

// ============================================================
// Post slug helpers (existing)
// ============================================================

/**
 * Build a Set of post IDs that have English translations.
 * English posts live in src/content/posts/en/, so their IDs match
 * the Chinese post IDs directly.
 */
export function buildTranslationMap(enPostIds: string[]): Set<string> {
  return new Set(enPostIds.map(id => enPostSlug(id)));
}

/**
 * Get the slug for linking. English post ID is already the correct slug.
 */
export function enPostSlug(enPostId: string): string {
  return enPostId;
}
