 ...
 *
 * Same recipe as grove3d/Sakura.tsx:
 *   - Per-tree seeded mulberry32 → unique structure per call
 *   - Trunk: single-mesh CatmullRom-curved tapered tube (no segment gaps)
 *   - Main / secondary / twig branches: QuadraticBezier + tapered tube
 *   - Petals: shared canvas alphaMap, ~10K instances per hero on a single
 *     InstancedMesh (1 draw call for the entire petal cloud)
 *   - Hero mode = larger trunk, more main branches, wider canopy, more petals
 *   - Wind sway = group-level X/Z rotation (1 transform per frame)
 *
 * Vendored copy so /world/ owns its visual destiny. If we want to evolve
 * the sakura look here without breaking grove3d, edit this file directly.
 */
