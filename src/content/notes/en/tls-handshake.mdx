...
immersive:
  url: /en/immersive/tls-handshake/
  label: Immersive
---

> This is an immersive long-form essay — best read in its dedicated layout → [Open the full version](/en/immersive/tls-handshake/)

The seventh entry in the **Field Note** series. The sister piece, «[The Life of One Request · HTTP/3 in Full](/en/immersive/http3/)», treated TLS as "a passenger that QUIC ferries via CRYPTO frames." This time the camera turns around and points at the passenger itself — TLS 1.3, byte-by-byte from the first octet of `ClientHello`, how HKDF grows eight keys from one root seed, how the certificate chain is verified, how ECH uses HPKE to wrap the SNI completely, and how post-quantum KEM squeezes into the `key_share` slot.
